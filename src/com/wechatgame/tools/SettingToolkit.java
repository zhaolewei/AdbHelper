...
public abstract class SettingToolkit {
    ...
    private final static String PRO_README_FILE_NAME = "readme.txt";
    private final static String PRO_SETTING_FILE_NAME = "setting.properties";
    private final static String PRO_ADB_PATH = "adb"; //adb工具路径配置项
    ...
    static {
        defaultResolutionMapJumpRate.put("1600*2560",0.92*4);
        defaultResolutionMapJumpRate.put("1440*2960",1.02*4);
        defaultResolutionMapJumpRate.put("1440*2560",1.475*2.5); //ok
        defaultResolutionMapJumpRate.put("1080*2220",1.392*1.5);
        defaultResolutionMapJumpRate.put("1080*2160",1.372*2.5);

        defaultResolutionMapJumpRate.put("1080*1920",1.392*1);//ok
        //defaultResolutionMapJumpRate.put("1080*1920",1.475*2.5); //小米5x，fixed
        defaultResolutionMapJumpRate.put("720*1440",2.078*2);//ok
        defaultResolutionMapJumpRate.put("720*1280",2.078*2);//ok
        defaultResolutionMapJumpRate.put("540*960",2.732*2);//ok
    }
    ...
    public static void init(){
        ...
        File readFile = new File(PRO_README_FILE_NAME);
        if(!readFile.exists()){
            try {
                boolean r =readFile.createNewFile();
                if(r) {
                    //初始化说明信息
                    FileOutputStream newFileOutputStream = new FileOutputStream(PRO_README_FILE_NAME);
        ...
        File file = new File(PRO_SETTING_FILE_NAME);
        if(!file.exists()){
            try {
                ...
                if(r) {
                    ...
                    FileOutputStream newFileOutputStream = new FileOutputStream(PRO_SETTING_FILE_NAME);
                    Properties properties = new Properties();
                    properties.setProperty(PRO_ADB_PATH,"adb");
                    properties.setProperty(PRO_UI_RATE,"1080*1920");
                    properties.setProperty(PRO_JUMP_RATE,"1080*1920");
                    properties.store(newFileOutputStream, "app setting");

                    newFileOutputStream.flush();
                    newFileOutputStream.close();
                    LogToolKit.println("成功创建并初始化配置文件 "+jarPath+ PRO_SETTING_FILE_NAME);
                }else{
    ...
    public static void setADBPath(String path){
        ...
        try {
            oFile = new FileOutputStream(PRO_SETTING_FILE_NAME);
            pro.store(oFile, "app setting");
            oFile.close();
            LogToolKit.println("保存ADB地址配置信息到 "+ PRO_SETTING_FILE_NAME);
    ...
    private static Properties getSettingProperties(){
        ...
        File file = new File(PRO_SETTING_FILE_NAME);
        if(file.exists()){
            //LogToolKit.println("读取配置文件 "+PRO_SETTING_FILE_NAME);
            try {
                FileInputStream in = new FileInputStream(PRO_SETTING_FILE_NAME);
                pro.load(in);
                in.close();
            } catch (IOException e) {
                e.printStackTrace();
            ...
            try {
                ...
                if(r) {
                    ...
                    pro.load(in);
                    in.close();
                    LogToolKit.println("成功创建配置文件 "+ PRO_SETTING_FILE_NAME);
                }else{
                    LogToolKit.println(" 配置文件创建失败！");
                }
            } catch (IOException e) {
