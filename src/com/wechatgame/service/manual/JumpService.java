package com.wechatgame.service.manual;
...
public abstract class JumpService {
	...
	public static boolean jump(Point beginPoint, Point endPoint) {
		...
		if (d != 0) {
			...
			// return false;
			// }
			// AdbToolKit.screentouch(Math.floor(d *
			// (SettingToolkit.getJumpRate() / SettingToolkit.getUiRate()) ));
			try {
				Thread.sleep(1500);
	...
	public static void setBeginPoint(Point beginPoint) {
		if (beginPoint != null) {
			JumpService.beginPoint.setLocation(beginPoint.getX(),
					beginPoint.getY());
			if (beginPoint.getX() > 0 && beginPoint.getY() >= 0) {
				LogToolKit.println("起跳点 (" + beginPoint.getX() + ", "
						+ beginPoint.getY() + ")");
			}
		} else {
	...
	public static void setEndPoint(Point endPoint) {
		if (endPoint != null) {
			...
			if (endPoint.getX() > 0 && endPoint.getY() >= 0) {
				LogToolKit.println("目标点 (" + endPoint.getX() + ", "
						+ endPoint.getY() + ")");
			}
		} else {
