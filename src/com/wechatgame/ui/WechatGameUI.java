...
import com.wechatgame.service.manual.JumpService;
	...
	private static void initWidthHeight() {
		...
		if (image != null) {
			...
			if (SettingToolkit.getUiRate() == null) {
				Double tempUIRate = SettingToolkit
						.getDefaultUIRatioByResolution(
								resulotionStr.toString());
				if (tempUIRate != null) {
					SettingToolkit.setUiRate(tempUIRate);
					LogToolKit.println(
							"使用系统自适配的UI适配率：" + SettingToolkit.getUiRate());
				} else {
					SettingToolkit.setDefaultTempUiRate();
					LogToolKit.println("目前系统没有该分辨率的UI适配率，使用固定默认值("
							+ SettingToolkit.getUiRate()
							+ ")或者请到【setting.properties】文件内手动配置【ui_rate】参数，参数值可以参考【README.TXT】");
				}
			} else {
				LogToolKit
						.println("使用设置文件中的UI适配率：" + SettingToolkit.getUiRate());
			}
			LogToolKit.println("设置窗体大小比率为：" + SettingToolkit.getUiRate());

			if (SettingToolkit.getJumpRate() == null) {
				Double jumpRate = SettingToolkit.getDefaultJumpRateByResolution(
						resulotionStr.toString());
				if (jumpRate != null) {
					SettingToolkit.setJumpRate(jumpRate);
					LogToolKit.println(
							"使用系统自适配的UI适配率：" + SettingToolkit.getJumpRate());
				} else {
					SettingToolkit.setDefaultTempJumpRate();
					LogToolKit.println("目前系统没有该分辨率的跳跃系数，使用固定默认值("
							+ SettingToolkit.getJumpRate()
							+ ")或者请到【setting.properties】文件内手动配置【jump_rate】参数，参数值可以参考【README.TXT】");
				}
			} else {
				LogToolKit.println(
						"使用设置文件中的跳跃系数：" + SettingToolkit.getJumpRate());
			}
			...
			return new MouseListener() {
				public void mouseClicked(MouseEvent e) {
					...
					if (MouseEvent.BUTTON1 == e.getButton()) {
						if (JumpService.getBeginPoint().getX() == 0
								&& JumpService.getBeginPoint().getY() == 0) {
							beginLable.setText("起跳点：(" + e.getPoint().getX()
									+ "," + e.getPoint().getY() + ")");
							JumpService.setBeginPoint(e.getPoint());
						} else {
							endLable.setText("目标点：(" + e.getPoint().getX() + ","
									+ e.getPoint().getY() + ")");
							JumpService.setEndPoint(e.getPoint());
							new Thread(new Runnable() {
								public void run() {
									JumpService.jump(
											JumpService.getBeginPoint(),
											JumpService.getEndPoint());
									refreshCorePanelUI();
